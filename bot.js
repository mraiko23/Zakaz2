...
// ============ CONFIGURATION ============
const BOT_TOKEN = '7880352029:AAFYoPqcD-6kNFJheR0HR23VQ2yBPFsUAsA';
...
async function checkSubscription(userId, channelId) {
  try {
    ...
    console.log(`[SUB] User ${userId} channel ${channelId}: ${member.status} = ${isSubscribed}`);
    return isSubscribed;
  } catch (error) {
    console.error(`[SUB ERROR] User ${userId} channel ${channelId}:`, error.message);
    return false;
  }
}
...
async function checkAllRequiredChannels(userId) {
  ...
  for (const channel of db.opChannels) {
    ...
    results.push({ channelId: channel.id, channel: channel.name, subscribed: isSubscribed });
  }
  return results;
}
...
function getRobuxKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: '🎫 Промокоды', callback_data: 'activate_promo' }],
...
function tasksKeyboard(page = 0, userId = null) {
  const tasksPerPage = 5;
  
  // Filter out completed tasks for this user
  let availableTasks = db.tasks;
  if (userId) {
    const user = getUser(userId);
    availableTasks = db.tasks.filter(task => !user.completedTasks.includes(task.id));
  }
  
  const start = page * tasksPerPage;
  const end = start + tasksPerPage;
  const pageTasks = availableTasks.slice(start, end);
  ...
  keyboard.push([{ text: '« Назад', callback_data: 'get_robux' }]);
...
function taskDetailKeyboard(taskId) {
  ...
  task.channels.forEach((channel, index) => {
    const buttonLabel = channel.buttonLabel || `${index + 1}. ${channel.name}`;
    keyboard.push([{ text: buttonLabel, url: channel.url }]);
  });
...
function adminMenuKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: '⚠️ Текст подписки на каналы', callback_data: 'admin_edit_subscribe_text' }],
      ...
      [{ text: '📚 Подсказки по командам', callback_data: 'admin_help_commands' }],
      [{ text: '« Назад', callback_data: 'main_menu' }]
    ]
  };
}
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  if (db.opChannels.length > 0) {
    ...
    if (unsubscribed.length > 0) {
      const subscribeText = db.settings.subscribeText || '⚠️ Для использования бота подпишитесь на обязательные каналы:';
      
      // Show only channels user is NOT subscribed to (by ID)
      const unsubscribedIds = unsubscribed.map(s => s.channelId);
      const channelsToShow = db.opChannels.filter(ch => unsubscribedIds.includes(ch.id));
      
      const keyboard = {
        inline_keyboard: [
          ...channelsToShow.map(ch => [{ text: `📢 ${ch.name}`, url: ch.url }]),
          [{ text: '✅ Проверить подписку', callback_data: 'check_subscriptions' }]
        ]
      };
      
      return bot.sendMessage(chatId, subscribeText, { reply_markup: keyboard });
    }
  }
...
bot.on('message', async (msg) => {
  ...
  // Check required channels
  if (db.opChannels.length > 0 && text !== 'ℹ️ О боте') {
    const subscriptions = await checkAllRequiredChannels(userId);
    const unsubscribed = subscriptions.filter(s => !s.subscribed);
  ...
  if (text === '💰 Получить Робуксы') {
    ...
    bot.sendMessage(chatId, message, { reply_markup: getRobuxKeyboard() });
  }
    ...
    for (const uid in db.users) {
      ...
      totalReferrals += u.referrals ? u.referrals.length : 0;
    }
    
    if (db.withdrawalRequests) {
      db.withdrawalRequests.forEach(req => {
    ...
    let message = db.settings.aboutText;
    message += `\n\n📊 Статистика бота:\n`;
    message += `👥 Пользователей в боте: ${totalUsers}\n`;
    message += `💸 Выведено: ${totalWithdrawn} Робуксов\n`;
    message += `🔗 Рефералов: ${totalReferrals}`;
    
    bot.sendMessage(chatId, message, { reply_markup: aboutBotKeyboard() });
  }
...
bot.on('callback_query', async (query) => {
  ...
  if (!data.startsWith('admin_') && !data.startsWith('delete_op_') && !data.startsWith('delete_task_') && !data.startsWith('delete_promo_') && !data.startsWith('activate_promo') && data !== 'check_subscriptions' && data !== 'main_menu' && data !== 'about_bot') {
    if (db.opChannels.length > 0) {
      ...
      if (unsubscribed.length > 0) {
        ...
        bot.editMessageText(subscribeText, {
          chat_id: chatId,
  ...
  // Main menu handlers
  if (data === 'main_menu') {
    // Delete the inline message and send new message with reply keyboard
    try {
    ...
    // Return to get_robux menu with profile
    const user = getUser(userId);
    const referralLink = `https://t.me/${(await bot.getMe()).username}?start=${userId}`;
    ...
    bot.editMessageText('🎫 Введите промокод:\n\n💡 Промокоды дают бонусы к выводу или робуксы на баланс', {
      chat_id: chatId,
      message_id: messageId,
      reply_markup: { inline_keyboard: [[{ text: '❌ Отмена', callback_data: 'promo_cancel' }]] }
    });
    db.adminStates = db.adminStates || {};
    db.adminStates[userId] = { action: 'enter_promo', messageId: messageId };
    saveDB(db);
  }
    ...
    try {
      ...
      // Calculate total withdrawals and referrals
    ...
    // Filter out completed tasks for this user
    const user = getUser(userId);
    const availableTasks = db.tasks.filter(task => !user.completedTasks.includes(task.id));
    
    const message = availableTasks.length > 0 
    ...
    bot.editMessageText(message, {
      chat_id: chatId,
      message_id: messageId,
      reply_markup: tasksKeyboard(0, userId)
    });
  }
    ...
    bot.editMessageText('📋 Доступные задания\n\nВыберите задание для выполнения:', {
      chat_id: chatId,
      message_id: messageId,
      reply_markup: tasksKeyboard(page, userId)
    });
  }
  ...
  // Delete promo handler (must be before admin_ block)
    ...
    if (promo) {
      ...
      if (db.promocodes.length === 0) {
        ...
        message += '📝 Команды:\n';
        message += '/add_promo КОД процент кол-во\n(бонус % к выводу)\n\n';
        message += '/add_promo_rb КОД робуксы кол-во\n(робуксы на баланс)\n\n';
        message += '💡 Примеры:\n/add_promo BONUS20 20 100\n/add_promo_rb GIFT10 10.5 1000';
      } else {
        ...
        db.promocodes.forEach((promo, i) => {
          ...
          if (promo.type === 'robux') {
            message += `   💰 Робуксов: ${promo.robuxAmount}\n`;
          } else {
        ...
        message += '\n📝 Добавить:\n/add_promo КОД % кол-во\n/add_promo_rb КОД робуксы кол-во';
      }
  ...
  // Admin handlers
    ...
    if (data === 'admin_broadcast') {
      ...
      if (db.promocodes.length === 0) {
        ...
        db.promocodes.forEach((promo, i) => {
          ...
          message += `${i + 1}. 🎫 ${promo.code}\n`;
          if (promo.type === 'robux') {
            message += `   � иРобуксов: ${promo.robuxAmount}\n`;
          } else {
            message += `   📈 Бонус: +${promo.bonus}%\n`;
          }
          message += `   👥 Использовано: ${usedCount}/${promo.maxUses}\n\n`;
    ...
    else if (data === 'admin_edit_subscribe_text') {
      const currentText = db.settings.subscribeText || '⚠️ Для использования бота подпишитесь на обязательные каналы:';
      bot.sendMessage(chatId, `⚠️ Текущий текст подписки:\n"${currentText}"\n\nВведите новый текст:`, { reply_markup: { inline_keyboard: [[{ text: '❌ Отмена', callback_data: 'admin_cancel' }]] }});
      db.adminStates = db.adminStates || {};
      db.adminStates[userId] = { action: 'edit_subscribe_text' };
      saveDB(db);
    }
    
    else if (data === 'admin_help_commands') {
      let helpText = `📚 Подсказки по командам\n\n`;
      helpText += `🎫 Промокоды:\n`;
      helpText += `/add_promo КОД процент кол-во\n`;
      helpText += `Пример: /add_promo BONUS20 20 100\n`;
      helpText += `(+20% к выводу, 100 использований)\n\n`;
      helpText += `/add_promo_rb КОД робуксы кол-во\n`;
      helpText += `Пример: /add_promo_rb #GIFT10 10.5 1000\n`;
      helpText += `(10.5 робуксов на баланс, 1000 использований)\n\n`;
      helpText += `📋 Задания:\n`;
      helpText += `/add_task - создать задание\n`;
      helpText += `/set_task_channel_label ID_задания номер_кнопки текст\n`;
      helpText += `Пример: /set_task_channel_label 1 1 Подпишись!\n\n`;
      helpText += `⭐ ОП каналы:\n`;
      helpText += `/add_op_channel @username Название\n`;
      helpText += `/add_op_channel -1001234567890 Название\n`;
      helpText += `/add_op_channel https://t.me/+hash -1001234567890 Название\n\n`;
      helpText += `🔑 Токены вывода:\n`;
      helpText += `/stoptoken ТОКЕН - завершить вывод\n`;
      helpText += `/checktoken ТОКЕН - проверить статус`;
...
bot.on('message', async (msg) => {
  ...
  else if (adminState.action === 'edit_subscribe_text' && isAdmin(username)) {
    db.settings.subscribeText = msg.text;
    saveDB(db);
    bot.sendMessage(chatId, `✅ Текст подписки обновлен:\n"${msg.text}"`);
    delete db.adminStates[userId];
  }
  ...
  // Handle withdrawal amount input from users
    ...
    // Check promo type
    if (promo.type === 'robux') {
      // Direct robux promo - add to balance immediately
      user.balance += promo.robuxAmount;
      user.usedPromocodes.push(promoCode);
      saveDB(db);
      
      bot.sendMessage(chatId, `✅ Промокод активирован!\n\n🎫 Код: ${promoCode}\n💰 Получено: +${promo.robuxAmount} Робуксов\n\n💵 Ваш баланс: ${user.balance} Робуксов`);
    } else {
      // Percentage bonus promo - save for withdrawal
...
название | описание | награда | каналы
...
Примеры:
Подписка | Подпишитесь! | 50 | @channel1,@channel2
Приватный | Подпишитесь! | 10 | https://t.me/+abc123,-1001234567890`);
...
// Supports codes with #, !, _ and other special chars
bot.onText(/\/add_promo ([^\s]+) (\d+) (\d+)/, async (msg, match) => {
  const username = msg.from.username;
  if (!isAdmin(username)) {
...
// Add promo for direct robux balance (not percentage)
// Supports codes with #, !, _ and other special chars
bot.onText(/\/add_promo_rb ([^\s]+) ([\d.]+) (\d+)/, async (msg, match) => {
  const username = msg.from.username;
  if (!isAdmin(username)) {
    return bot.sendMessage(msg.chat.id, '⛔ У вас нет прав администратора');
  }
  ...
  const code = match[1].toUpperCase();
  const robuxAmount = parseFloat(match[2]);
  const maxUses = parseInt(match[3]);
  ...
  if (robuxAmount <= 0) {
    return bot.sendMessage(msg.chat.id, '❌ Количество робуксов должно быть больше 0');
  }
  ...
  bot.sendMessage(msg.chat.id, `✅ Промокод на робуксы создан!\n\n🎫 Код: ${code}\n💰 Робуксов: ${robuxAmount}\n👥 Макс использований: ${maxUses}\n\n💡 Пользователи могут активировать его в разделе "Получить Робуксы" -> "Промокоды"`);
});

// Set task channel button label
// Format: /set_task_channel_label taskId channelIndex buttonText
bot.onText(/\/set_task_channel_label (\d+) (\d+) (.+)/, async (msg, match) => {
  const username = msg.from.username;
  if (!isAdmin(username)) {
    return bot.sendMessage(msg.chat.id, '⛔ У вас нет прав администратора');
  }
  ...
  const channelIndex = parseInt(match[2]) - 1; // Convert to 0-based index
  const buttonLabel = match[3].trim();
  
  const task = db.tasks.find(t => t.id === taskId);
  
  if (!task) {
    return bot.sendMessage(msg.chat.id, `❌ Задание с ID ${taskId} не найдено`);
  }
  
  if (channelIndex < 0 || channelIndex >= task.channels.length) {
    return bot.sendMessage(msg.chat.id, `❌ Канал с номером ${channelIndex + 1} не найден в задании\n\nВ задании ${task.channels.length} каналов`);
  }
  
  task.channels[channelIndex].buttonLabel = buttonLabel;
  saveDB(db);
  
  bot.sendMessage(msg.chat.id, `✅ Текст кнопки обновлен!\n\n📋 Задание: ${task.title}\n📢 Канал #${channelIndex + 1}: ${task.channels[channelIndex].name}\n🔘 Новый текст кнопки: ${buttonLabel}`);
});
...
bot.on('message', async (msg) => {
  ...
  if (adminState && adminState.action === 'create_task' && isAdmin(username)) {
    try {
      ...
      // Smart parsing: split by comma, but rejoin parts that look like link,-chatId
      const rawParts = channelsStr.split(',').map(c => c.trim()).filter(c => c);
      ...
      for (let i = 0; i < rawParts.length; i++) {
        ...
        // Check if next part is a negative number (chat ID) and current part looks like a link
        if (nextPart && nextPart.match(/^-\d+$/) && (part.includes('t.me/') || part.includes('http'))) {
          // Combine link with chat ID
          channelUsernames.push(`${part},-${nextPart.substring(1)}`);
          i++; // Skip next part
        } else {
          channelUsernames.push(part);
        }
      }
      ...
      for (const chInput of channelUsernames) {
        try {
          ...
          // Check if it's in format: link,id (e.g., https://t.me/+hash,-1001234567890)
          // Look for the last comma followed by a negative number (chat ID)
          const lastCommaMatch = chInput.match(/^(.+),(-\d+)$/);
          if (lastCommaMatch) {
            const link = lastCommaMatch[1].trim();
            const id = lastCommaMatch[2].trim();
